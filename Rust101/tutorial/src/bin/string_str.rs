...
(String type 2)In Rust, the String type is a growable, mutable, owned, UTF-8 encoded string type. It is different from a string literal, which is a string slice (&str) that is stored in the binary of the program and is not growable or mutable.
...
//&str is an immutable reference to a part of a string.
// &str type  String literal
1.String-- owned
...
//TYPES OF STRING
// 1. String Literal = "hello" (fixed sized ,compile time constant. written as sequence of characters with double qoutes). the type is = &str

//String(`string`). this type represents an owned, heap allocated string. it is more flexible than &str and allows dynamic modifications. an example being String::from()